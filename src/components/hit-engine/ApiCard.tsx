    ...
    <div className={`rounded-xl border p-4 space-y-3 transition-all ${api.enabled ? 'border-pink-500/30 bg-gray-950/80' : 'border-gray-800 bg-gray-950/40 opacity-60'}`}>
      ...
      <div className="flex gap-2">
        <button onClick={onEdit}
          className="flex-1 py-2 rounded-xl bg-gray-800/80 border border-gray-700 text-gray-300 text-xs font-mono font-bold hover:bg-gray-700 transition-colors flex items-center justify-center gap-2">
          <Edit2 className="w-3.5 h-3.5" /> Edit
        </button>
        <button onClick={onDelete}
          className="py-2 px-4 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 text-xs font-mono font-bold hover:bg-red-500/20 transition-colors flex items-center justify-center gap-2">
